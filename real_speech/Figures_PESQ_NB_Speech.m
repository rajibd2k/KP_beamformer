%%% PESQ (narrowband) for kronecker fixed /+ adaptive beamforming
%%% Varying iSIR
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for idx_intf_type = 1:length(intf_type) 
    ...
    for idx_iSIR_dB = 1 : length(iSIR_dB_range)
        ...
        for idx_dataset = [1:8,11:18] %% 1:100
            ...
            Metric = Metric.PESQ_NB_BB ; 
            Metric = Metric(1,:) ; % broadband PESQ
            ...
            Metric = load([filters_dir,'/', filter , postname]) ; 
            Metric = Metric.PESQ_NB_BB ; 
            Metric = Metric(2,:) ; % broadband PESQ 
            Metric = reshape( Perf(idx_iSIR_dB,idx_filter,:), 1, 2 ) + Metric ;
            Perf(idx_iSIR_dB,idx_filter,:) = Metric ;
...
save('PESQ_NB', 'Perf_Metrics') ;

clear all ; clc ; close all ;
iSIR_dB_range = [-10:5:20] ;
num_filters = 5 ;
intf_type = {'white', 'babble', 'hfchannel'} ;
Perf_Metrics = load('PESQ_NB') ;
...
for idx_intf_type = 1:length(intf_type)  
    ...
    xlabel('iSIR (dB)') ; ylabel('PESQ-NB (dB)') ; title(intf_type{idx_intf_type}) ; axis('tight') ;
