%% MVDR (Minimum Variance Distortionless Response) Kronecker Filter Beamforming 
...
function [h] = MVDR_Kronecker_error( est_phiY, d_1, d_2, n_iter )
...
if n_iter == 0
    for idxf=1:length(f)
        phiY_2(:,:,idxf) = kron( I_M1,h_2(:,idxf) )' * est_phiY(:,:,idxf) * kron( I_M1,h_2(:,idxf) );
%         h_1(:,idxf) = inv( phiY_2(:,:,idxf) )*d_1(:,idxf)/(d_1(:,idxf)'*inv( phiY_2(:,:,idxf) )*d_1(:,idxf));
        try
            h_1(:,idxf) = pinv( phiY_2(:,:,idxf) )*d_1(:,idxf)/(d_1(:,idxf)'*pinv( phiY_2(:,:,idxf) )*d_1(:,idxf));
        catch
            h_1(:,idxf) = inv( phiY_2(:,:,idxf) )*d_1(:,idxf)/(d_1(:,idxf)'*inv( phiY_2(:,:,idxf) )*d_1(:,idxf));
        end
